...
data <- rbindlist( lapply(symbols[, symbol], function(x) load_with_ma(x, symbols[symbol==x, start_date])))
data[, symbol_f := factor(symbol, levels=unique(symbol))]  # Fix the ordering of the symbols so that facet_grid doesn't reorder alphabetically
